...
All pre-assembly steps (1–3) start as active (● blue). The assembly step (4) starts pending (○ gray) until all prior steps are completed. The first step is expanded by default.

**Layout (expanded — one step selected):**
```
│ ● 1. Find the definition                              ▼    │
│   ┌─────────────────────────────────────────────┐           │
...
Clicking any other active step (e.g. step 2 or 3) switches the expansion to that step.

**State Indicators (4 visual states):**
- ○ = Pending (gray hollow circle) — only assembly before it's unlocked
- ● = Active + collapsed (blue filled circle, clickable) — available but not expanded
- ● ▼ = Active + expanded (blue filled circle, chevron, content visible) — the step the user is working on
- ✓ = Completed (green checkmark, completion text shown)

**Non-Linear Step Completion:**

A skilled cryptic solver spots the most obvious clue element first — forcing a prescribed order is artificial. The step system reflects this:

1. **All pre-assembly steps are available simultaneously** — the student can tackle definition, indicators, and wordplay type in any order
2. **Only one step is expanded at a time** — clicking an active step header switches the expansion (sends `select_step` action to server)
3. **Assembly is gated** — the assembly step becomes active only when ALL prior steps are completed
4. **Auto-advance on completion** — when a step is answered correctly, the expansion automatically moves to the next uncompleted active step
5. **`step_index` is the expanded-step pointer** — it tracks which step is currently shown, not which steps are accessible
6. **`_is_step_available()`** determines availability: assembly requires all prior indices in `completed_steps`; all other steps are always available
7. **`_next_active_step()`** finds the next uncompleted available step (scanning forward, wrapping) after a correct answer

**Behavior:**
1. On session start, step 0 is expanded (collapsed by default — user clicks chevron to expand)
2. Expanded step shows word chips, prompt, intro, hint 💡
3. On correct input → step completes (green ✓), next uncompleted active step auto-expands
4. Clicking any active (non-completed, non-pending) step header switches expansion to it
5. Completed steps show green ✓ with completion text below
6. Answer boxes always visible at top, editable until locked
7. When all pre-assembly steps completed → assembly step becomes active
8. When all steps completed → completion view

**Data Source:**
- Step list generated by `_build_step_list()` from `clue["steps"]` array
- Step status determined by `_is_step_available()` — not by `step_index` position
...
- `select_step` action in `update_ui_state()` handles step switching (resets per-step UI state)
...
3. **Set environment variables** in Vercel:
   ...
   vercel env add SESSION_SECRET      # 64-char hex key for HMAC session signing
   ```
...
`IS_PRODUCTION = bool(os.environ.get("VERCEL"))` — Vercel sets `VERCEL=1` automatically. This flag gates debug-only features so they are never exposed in production.

### 15.2 Stack Trace Suppression

In production, API error responses return only `{"error": "...message..."}` without stack traces. Full tracebacks are logged server-side via `app.logger.error()` for debugging but never sent to the client. In development, tracebacks are included in the response for convenience.

Applies to both upload error handlers in `crossword_server.py`.

### 15.3 Server Info Endpoint

The `/server-info` route (returns working directory, Python version, loaded modules) is disabled in production — returns `{}` immediately. The client-side inline script guards against missing data with `if(d.dir)`.

### 15.4 XSS Prevention

All server-sourced strings rendered via `innerHTML` in `crossword.js` are escaped through a shared `escapeHTML()` helper (DOM-based: sets `textContent`, reads back `innerHTML`). This prevents stored XSS if clue text or puzzle metadata contains HTML/script tags.

Applied to: clue text display (2 locations), puzzle list rendering, error messages (2 locations), warning displays, reconciliation log entries (3 locations).

`trainer.js` already used `textContent` throughout — no changes needed.

### 15.5 HMAC Session Signing

Client-carried sessions are signed with HMAC-SHA256 to prevent tampering. The session format is `{"data": {...}, "sig": "hex"}`.

**Signing (`_sign_session`):** Serialises session data with `json.dumps(sort_keys=True, separators=(',', ':'))`, computes HMAC with `SESSION_SECRET`, returns `{data, sig}`.

**Verification (`_verify_session`):** Recomputes the HMAC and uses `hmac.compare_digest()` (constant-time comparison) to reject tampered sessions. Invalid sessions return 400 to the client.

**Key management:**
- Production: `SESSION_SECRET` environment variable (64-char hex string, set in Vercel dashboard)
- Development: If `SESSION_SECRET` is not set, a random key is generated at startup with a console warning. Sessions won't survive server restarts in this case.
- Generate with: `python3 -c "import secrets; print(secrets.token_hex(32))"`

### 15.6 Secure File Uploads

Uploaded filenames are sanitised with `werkzeug.utils.secure_filename()` to prevent path traversal attacks (e.g. `../../etc/passwd`). If `secure_filename()` returns an empty string (all characters stripped), the upload is rejected with 400.
